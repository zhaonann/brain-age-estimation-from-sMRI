...
from models.threedim import Threedim_CNN
import utils
...
def main():
    ...
    args.parsave = "{}-{}".format('threedim', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "threedim.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_threedim.py")))
        ...
        # model      
        model_s = Threedim_CNN(in_channels=1).to(device) 
        # ResNet-18 or ResNet-34
        # model_s = eval('resnet'+str(args.model_depth)+'()').to(device)  
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for data in train_queue:
                sMRI, labels = data[:2] # label is for chronological age
                ...
                loss_s = 0.5*loss_s # coefficient of 0.5 to counteract the square from gradient
...
def preds_store(model_s, queue, n_datas, stage="val", device=None, path_figs=None, exp=0):
    ...
    plt.savefig(os.path.join(path_figs, "exp_%d_%s_MAE_%.2f_r_%.3f_RMSE_%.2f" % (
        exp, stage, mae_s, r, rmse) + ".png")) # or .eps
