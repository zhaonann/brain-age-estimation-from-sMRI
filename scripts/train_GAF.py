import time
from models.GAF_model import GAF_MLP
import utils
from data import ExCustomDataset_GAF, data_split
...
def main():
    ...
    data_csv = "/public_bme/share/sMRI/csv/subinfo_HCs_8_DK106.csv" # path to data
    outpath =os.path.join(pathpar, 'OUTPUT')
    if not os.path.exists(outpath):
        os.mkdir(outpath)   

    args.parsave = "{}-{}".format('GAF', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "GAF_model.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_GAF.py")))
        ...
        dataset = ExCustomDataset_GAF(df=df)
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for data in train_queue:
                cli_data, labels, sex_gt = data[:3]
                cli_data = cli_data.to(device)
                labels = labels.unsqueeze(1).to(device)
                sex_gt = sex_gt.to(device)

                optimizer_s.zero_grad()

                output_s = model_s(cli_data)
        ...
        logging.info("*******Again Last Epoch %d,  Val MAE %.2f,  In Test MAE %.2f*******",args.n_epochs, val_mae_s, in_test_mae_s)
        ...
        logging.info("******* Best Epoch %d, Train MAE %.2f, Val MAE %.2f, In Test MAE %.2f *******", val_opt["Epoch"], val_opt["Opt_train"], val_opt["Opt_val"], in_test_mae_s)
...
def preds_store(model_s, queue, n_datas, stage="val", device=None, path_figs=None, exp=0):
    ...
    with torch.no_grad():
        for data in tqdm(queue):
            GAF, labels, sex, idx, filename, site = data
            GAF = GAF.to(device)
            labels = labels.unsqueeze(1).to(device)
            sex = sex.to(device)

            output_s = model_s(GAF)
