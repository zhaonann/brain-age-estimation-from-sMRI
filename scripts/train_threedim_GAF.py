import time
from models.ResNet3D import resnet10, resnet18, resnet34, resnet50, resnet101
from models.threedim_GAF_MSA import Threedim_GAF_CNN
import utils
from data import ExCustomDataset_threedim_GAF, data_split
...
def main():
    ...
    args.note = ""
    args.parsave = "{}-{}".format('threedim_GAF', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "threedim_GAF_MSA.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_threedim_GAF.py")))
        ...
        dataset = ExCustomDataset_threedim_GAF(df, transforms=True)
        ...
        dataset = ExCustomDataset_threedim_GAF(df, transforms=False)
        ...
        model_s = Threedim_GAF_CNN(in_channels=1).to(device)

        logging.info("sMRI, param size = %.3f MB", utils.count_parameters_in_MB(model_s))
        utils.print_networks(model_s, "sMRI")
        
        if args.pretrain_3D:
            # threedim_path = "/public/bme/home/zhaonan/brain_age_est/codes/OUTPUT/threedim-20230731-212100/exp_0/WEIGHTS/model_s_best_weight.pkl" # 100 epoch
            threedim_path = "/public/bme/home/zhaonan/brain_age_est/codes/BRAIN_AGE_ESTIMATION_CONFERENCE/OUTPUT/threedim-20230808-215808/exp_0/WEIGHTS/model_s_best_weight.pkl"
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            logging.info("******* Epoch %d, Train Loss MSE %.2f, Train MAE %.2f *******", epoch, train_loss, train_mae )
            logging.info("******* Epoch %d, Val Loss MSE %.2f, Val MAE %.2f *******", epoch, loss_dict['sMRI'], mae_dict['sMRI'])
...
def preds_store(model_s, queue, n_datas, stage="val", device=None, path_figs=None, exp=0):
    ...
    with torch.no_grad():
        for data in tqdm(queue):
            sMRI, GAF, labels, sex, idx, filename, site = data
            sMRI = sMRI.to(device)
            GAF = GAF.to(device)
            labels = labels.unsqueeze(1).to(device)
            sex = sex.to(device)
