...
from models.threedim_3view_MSA_share_encoder import Threedim_3view_CNN
import utils
from data import ExCustomDataset_threedim_3view, data_split
...
def main():
    ...
    args.note = ""
    args.parsave = "{}-{}".format('threedim_3view', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "threedim_3view_MSA_share_encoder.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_threedim_3view.py")))
        ...
        dataset = ExCustomDataset_threedim_3view(df, transforms=True)
        ...
        dataset = ExCustomDataset_threedim_3view(df, transforms=False)
        ...
        model_s = Threedim_3view_CNN(in_channels=1).to(device)
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for data in train_queue:
                ...
                sMRI, axial, sagittal, coronal, labels = data[:5]
                sMRI = sMRI.to(device)
                ...
                output_s = model_s(sMRI, axial, sagittal, coronal)
...
def preds_store(model_s, queue, n_datas, stage="val", device=None, path_figs=None, exp=0):
    ...
    with torch.no_grad():
        for data in tqdm(queue):
            sMRI, axial, sagittal, coronal, labels, sex, idx, filename, site = data
            sMRI = sMRI.to(device)
            axial = axial.to(device)
            sagittal = sagittal.to(device)
            coronal = coronal.to(device)
    ...
    df = pd.DataFrame({'index': idxs, 'filename': filenames, 'preds_s': preds_s,
                       'age': ages, 'diff': diffs, 'site': sites, 'sex': sexs})  #  0-> female, 1->male,
