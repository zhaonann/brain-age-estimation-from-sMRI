import time
from models.threeview_share_encoder import Threeview_CNN
import utils
from data import ExCustomDataset_3view, data_split
...
def main():
    ...
    args.note = ""
    args.parsave = "{}-{}".format('threeview', time.strftime("%Y%m%d-%H%M%S")) # spectral graph conv
    ...
    for exp in range(0, args.n_exps):
        ...
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "models", "threeview_share_encoder.py")))
        utils.create_exp_dir(args.save, scripts_to_save=glob.glob(os.path.join(pathpar1, "train_3view.py")))
        ...
        dataset = ExCustomDataset_3view(df, transforms=True)
        ...
        dataset = ExCustomDataset_3view(df, transforms=False)
        ...
        # model
        model_s = Threeview_CNN().to(device) # our_channels = n_classes, include bg
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for data in train_queue:
                ...
                axial, sagittal, coronal, labels = data[:4]
                ...
                output_s = model_s(axial, sagittal, coronal)
...
def preds_store(model_s, queue, n_datas, stage="val", device=None, path_figs=None, exp=0):
    ...
    with torch.no_grad():
        for data in tqdm(queue):
            axial, sagittal, coronal, labels, gender, idx, filename, site = data
            axial = axial.to(device)
            sagittal = sagittal.to(device)
            coronal = coronal.to(device)
            labels = labels.unsqueeze(1).to(device)
